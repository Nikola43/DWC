...
function PermiteSoloNumerosDecimalesComaNoPrimeraPosSigno(e) {
  ...
  if (c == 44)
    ...
    return ((c >= 48)  && (c <= 57));
			...
			function PermiteSoloNumerosLetrasMinusculasYmayusculasYCaracteresEspeciales(e) {
				var codigoCar = e.charCode || window.event.keyCode;
				if (((codigoCar >= 48) && (codigoCar <= 57)) || ((codigoCar >= 97) && (codigoCar <= 122)){
					
					return true;
				} 
				if((codigoCar >=35) && (codigoCar<=38)) || ((codigoCar >= 65) && (codigoCar <= 90)){
					return true;
				}
				
				if(codigoCar == 95 || codigoCar ==64 || codigoCar==63 || codigoCar==42){
			...
			function PermiteLetrasYEspacio(evento){
				var evento = evento || window.event;
				var codigoCar= evento.charCode || evento.keyCode;
				
				console.log(codigoCar)
				if(((codigoCar >= 97) &&  (codigoCar<=122)) || codigoCar == 32){
			...
			function compararFechas(fecha){
				...
				console.log("fecha hoy: dia "+dia_hoy+" mes "+mes_hoy+" año "+anio_hoy);
				console.log("fecha usuario: dia "+dia_fecha+" mes "+mes_fecha+" año "+anio_fecha);
